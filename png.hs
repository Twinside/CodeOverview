...
updateCrc = B.foldl' crcStep
    where crcStep :: Word32 -> Word8 -> Word32
          crcStep crcVal ch = (crcTab ! n) `xor` (crcVal `shiftR` 8)
              where n = fromIntegral (crcVal `xor` fromIntegral ch)

          crcTab :: Array Word8 Word32
          crcTab = array (0, 255)
                 . zip   [0..255]
                 $ map calculator [0..255] 
            where calculator n = foldl'
                        (\c _k -> if c .&. 1 == 1
                                then 0xedb88320 `xor` (c `shiftR` 1)
                                else c `shiftR` 1) n [(0::Int)..7]
...
pngGrayscale dat = unpack $ B.concat $ hdr : concat [ihdr, imgdat, iend]
     where height = fromIntegral $ length dat
           ...
           imgdat = chunk iDAT (Z.compress imgbits)
           imgbits = B.concat $ map lineFlatter dat
           iend = chunk iEND B.empty

           lineFlatter :: [Int] -> B.ByteString
           lineFlatter = B.pack . (0:) . map fromIntegral
...
png24bit dat = B.concat $ hdr : concat [ihdr, imgdat ,iend]
     where height = fromIntegral $ length dat
           ...
           imagedata = B.concat $ map lineFlatter dat
           iend = chunk iEND B.empty

           lineFlatter :: [(Int,Int,Int)] -> B.ByteString
           lineFlatter pixelList = B.pack
                                 . (0:)
                                 .  map fromIntegral
                                 $ concatMap (\(r,g,b) -> [r,g,b]) pixelList
...
png24bitAlpha dat = B.concat $ hdr : concat [ihdr, imgdat ,iend]
     where height = fromIntegral $ length dat
           ...
           lineFlatter :: [(Int,Int,Int,Int)] -> B.ByteString
           lineFlatter pixelList = B.pack
                                 . (0:)
                                 .  map fromIntegral
                                 $ concatMap (\(r,g,b,a) -> [r,g,b,a]) pixelList
